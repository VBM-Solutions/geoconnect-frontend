  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <Card><CardContent><h2 className="font-bold">Qualité détaillée</h2>{[['Échanges', kpi.qualiteEchanges], ['Délais', kpi.respectDelais], ['Rapport', kpi.qualiteRapport]].map(([label, value]) => <div key={label as string} className="mt-3 flex justify-between text-sm"><span>{label}</span><strong>{value == null ? '—' : `${Number(value).toFixed(1)} / 5`}</strong></div>)}</CardContent></Card>
      </section>
