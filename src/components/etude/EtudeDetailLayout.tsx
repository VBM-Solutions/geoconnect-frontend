...
export function EtudeDetailLayout({
  ...
  const projectTitle = formatProjectTitle(studyType, demande?.adresseProjet?.ville, demande?.adresseProjet?.codePostal);
...
function formatProjectTitle(studyType: string, ville?: string, codePostal?: string): string {
  return [studyType, ville || 'Ville non spécifiée', codePostal].filter(Boolean).join(' – ');
}
